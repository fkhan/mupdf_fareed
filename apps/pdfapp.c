...
void pdfapp_onmouse(pdfapp_t *app, int x, int y, int btn, int modifiers, int state)
{
	...
	else if (state == -1)
	{
		if (app->iscopying)
		{
			...
			if (app->selr.x0 < app->selr.x1 && app->selr.y0 < app->selr.y1)
				windocopy(app);
			//pdfapp_showpage(app, 0, 1); // kakai -- repaint page with highlight
		}
		if (app->ispanning)
			app->ispanning = 0;
	}
	...
	else if (app->iscopying)
	{
		...
		app->selr.y0 = MIN(app->sely, y);
		app->selr.y1 = MAX(app->sely, y);
		pdfapp_onhighlight(app); // partial repaint
		//if (app->selr.x1 % 10 == 0 && app->selr.y1 % 10) pdfapp_showpage(app, 0, 1); // full repaint
		winrepaint(app);
	}

}

void pdfapp_subimage(fz_pixmap *dst, fz_pixmap *src)
{
	fz_bbox sr, dr;
	unsigned char *sp, *dp;
        int x, y, w, h, n;
...
void
addpath(void *user, fz_matrix ctm, pdfapp_t *app)
{
	fz_device *dev = user;
	//fz_bbox clip;
	//fz_matrix tm, trm;
	...
	path = fz_newpath();
	fz_moveto(path, app->selr.x0, app->selr.y0);
	fz_lineto(path, app->selr.x1, app->selr.y0);
	fz_lineto(path, app->selr.x1, app->selr.y1);
	fz_lineto(path, app->selr.x0, app->selr.y1);
	fz_closepath(path);
	//fz_debugpath(path, 100);
	dev->fillpath(dev->user, path, fz_identity(), pdf_devicergb,  background, 1.0);
	fz_freepath(path);


	//memset(pix->samples, value, pix2->w * pix2->h * pix2->n);
}


void pdfapp_onhighlight(pdfapp_t *app)
{
	fz_error error;
	fz_device *idev, *tdev, *mdev;
	fz_pixmap *image;
	...
	//sbox.x0 = bbox.x0; // app->selr.x0 + bbox.x0;
	//sbox.y0 = bbox.y0; // app->selr.y0 + bbox.y0;
	sbox.x0 = app->selr.x0;// + bbox.x0;
	sbox.y0 = app->selr.y0;// + bbox.y0;
	sbox.x1 = app->selr.x1;// + bbox.x0;
	sbox.y1 = app->selr.y1;// + bbox.y0;
	...
	image = fz_newpixmapwithrect(pdf_devicergb, app->selr);
	fz_clearpixmap(image, 0xFF);
	//fz_clearpixmap2(app->image, image, 0xFF);
	idev = fz_newdrawdevice(image);
	idev->selection = app->selr;

	fz_executedisplaylist(list, idev, ctm);

addpath(idev, ctm, app);
winrepaint(app);
	fz_freedevice(idev);

	
pdfapp_subimage(app->image, image);
